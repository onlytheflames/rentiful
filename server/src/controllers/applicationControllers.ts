...
export const listApplications = async (
  ...
  try {
    ...
    // Check if a user id is a tenant or manager
    if (userId && userType) {
      if (userType === "tenant") {
    ...
    // Grabbing all the list of application depending on the user's role
    ...
    const formattedApplications = await Promise.all(
      applications.map(async (app) => {
        // Grab the lease relevant to the application (only one lease for each application)
...
export const createApplication = async (
  ...
  try {
    ...
    const property = await prisma.property.findUnique({
      where: { id: propertyId },
      select: { pricePerMonth: true, securityDeposit: true }, // Select these 2 columns from Property table to use in both application & lease in one transaction
    });
